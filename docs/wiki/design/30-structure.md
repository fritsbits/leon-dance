...
│   │   └── per locatie/                        own page per open atelier (venue+slot): Pianofabriek, Maison des Cultures
...
|---|---|---|
| **Project** | hasMany **Edition** (FK `project_id`) · belongs to `Participatieve performances` category | **Real model since 2026-05-29** (`slug`/`name`). Trajectory documentation fields: *proces · nazorg · continuïteit · evolutie*. *Mariage* now; *Kosmos 2027* when ready. NL/FR/EN. |
| **Edition** (NL *editie*) | belongsTo **Project** (`project_id`) · hasMany **Event** (`edition_id`) | **Renamed `Editie`→`Edition` 2026-05-29** (English class/table `editions`/Filament `EditionResource`; NL label, route name, `{editie}` param + view file unchanged). Own page · own *groep* · own typed dates. NL/FR/EN. **Inschrijving-control fields:** `inschrijving_open` (bool toggle — team sets via Filament) · `inschrijving_closes_at` (nullable date). Methods: `status()` · `isInschrijvingOpen()` · `inschrijvingClosesSoon()`. Scope: `openInschrijving()`. Route model-binding by slug (unknown → 404). |
| **Atelier** | belongsTo **Venue** · hasMany **Event** | `type: open\|school` recurring workshop with a standing **Venue** + day/time (built 2026-05-28, Phase-1; supersedes the "RecurringPractice" slug concept). The **open** ateliers = *Atelier Leon*, **school** = *Leon op school* — grouped by `type` into one overview page each. Each **open** atelier also has its **own detail page** (`slug`, e.g. `/atelier-leon/pianofabriek`; P-19, 2026-05-29), child of the Atelier Leon overview — logistiek + OSM-kaart + eerstvolgende-hier. Internal types *Leons White Page · Leon rond de tafel* are **parentless Events** (no Atelier), shown in the Agenda with a 1-line explainer. NL/FR/EN. |
| **Venue** (NL *locatie*) | hasMany **Atelier** · hasMany **Event** | A place — `name · area · address` (+ nullable `lat`/`lng` for maps, + `photo`/`photo_credit` exterior shot). Built 2026-05-28 (Phase-1); photo + coords managed via Filament `VenueResource` (2026-05-29). Drives the Atelier Leon "Waar en wanneer" block + the per-locatie detail page (kaart + buitenkant-foto). NL/FR/EN. |
| **Event** (Agenda) | belongsTo **Edition** OR **Atelier** (FK, exclusive by type) · belongsTo **Venue** | `type` enum: *open atelier · **klas** · repetitie · try-out · voorstelling · Leons White Page · Leon rond de tafel*. Fields: *title · lead (named facilitator) · venue_name (free-text fallback) · partners · starts_at · ends_at · is_public · notes* + FKs `atelier_id`/`edition_id`/`venue_id`. **Integrity guard (2026-05-29):** open_atelier/klas → `atelier_id` only; repetitie/try-out/voorstelling → `edition_id` only; internal types (LWP, rond de tafel) → neither. `venueLabel()` = related Venue name, else `venue_name`. Scopes `forAtelierType`/`forProject` (via `edition.project`). The `practice_slug`/`project_slug`/`editie_slug` strings were **dropped 2026-05-29** (FK cutover complete). ⚠️ Cross-checked vs. raw [25-agenda](../_archive/discovery/25-agenda.md) 2026-05-27 — added `klas` (21/65 entries ≈ 32%) + `lead`/`partners` (every real entry carries them; "Activity · Lead · Venue" are the Excel columns Kristin maintains). |
...
> **Slug→FK normalization — COMPLETE (2026-05-29).** Models promoted from slug-strings to real FKs. **Phase-1:** `Atelier` + `Venue`; `events.atelier_id`/`venue_id`. **Phase-2:** `Project` model; `Editie`→`Edition` (class/table/Filament); `editions.project_id`; `events.edition_id`; `events.venue`→`venue_name` + `Event::venue()`; repetities attach to an editie; a school `Atelier` parents klas events; an Event type↔parent integrity guard; and the `practice_slug`/`project_slug`/`editie_slug` strings **dropped**. Model naming = English class, Dutch label (Project · Edition · Atelier · Venue · Event ↔ project · editie · atelier · locatie · activiteit). Spec: [event-model-normalization](../superpowers/specs/2026-05-28-event-model-normalization-design.md) · plans [phase-1](../superpowers/plans/2026-05-28-event-model-phase1-venue-atelier.md) / [phase-2](../superpowers/plans/2026-05-28-event-model-phase2-project-edition-fk.md).
