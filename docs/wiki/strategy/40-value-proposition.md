...
> **Ownership (anti-overlap contract).** This page owns the **promise** (unique value +
> reason-to-believe + vs-current + register). The **person** →
> [Personas](20-personas.md); the **job** → [Jobs-to-be-done](30-jobs-to-be-done.md); the
> **sequence over time** → [User journey](50-user-journey.md). Reference by stable ID;
> **never restate** the person/job/flow.
...
> *The redesigned leon.dance is the one owned place where Leon proves — qualitatively, in
> its own voice rather than subsidiedossiertaal — that this is a real, methodical, durable
> practice that matters, and turns an offline spark into "yes, this is for me." It is the
...
- **Only-here** — the **decree itself counts an org's own communication channels as
  proof**; a site built *to the participatie rubric* is a differentiator the dance peers
  don't exploit (Debateville runs the portal; they don't).
- **RTB → asset** — method *Inviter/Mixing/Reversing/Affirming* (Janssens & Steyaert
  academic basis) → *meerwaarde* · recurring free weekly Atelier = documented
  process+aftercare+continuity · **Birds→INVITED→Mariage** = quality-evolution · het
  TheaterFestival selection (lineage) · tiered funder/partner wall · report shell · the
  **"16 not 5 years"** historiek.
- **Register pair** — *institutional* ("methodical, durable, fundable — the *participatie*
  functie, evidenced").
...
- **Unique value** — a professional dossier that makes Leon bookable — **three bookable
  things, not one**.
- **Only-here** — *Mariage* as a **replicable local-cast format** + the **mobiele
  dansstudio** (a bookable public-space studio peers don't have) — distinctive bookable
  *objects*, not just a company page.
- **RTB → asset** — Mariage 6 editions 2024–26 (core+local cast) · *Birds* tour-direction
  since 2023 (De Munt/STUK/tanzhaus nrw) · *Kosmos* 2027 (KANAL-Centre Pompidou/KVS) · the
  bike brochure · clear contact.
...
- **Only-here** — the cross-sector **"antenne" cluster** (cultuur+welzijn+zorg+onderwijs
  around one place) — a *way of working* a standard arts-in-school provider can't offer.
- **RTB → asset** — **partner-coordinator voice** (Schoemaker/Dewin/Meynaerts — the
  gatekeeper's *own* words) · cadence stated up front (weekly/2h/~6mo) · Leon carries
  logistics/space · it visibly serves their mandate (*"Leon"* on the Cardijnschool report
  cards) · clear contact.
- **Register pair** — *belonging × institutional hybrid* ("warm — and it serves your
...
- **Only-here** — the public-space, *"wordless common language"* social hook — the
  reliable press angle, distinctive vs conventional dance press.
- **RTB → asset** — structured agenda (participant view) · press-kit basics + the 58-photo
  archive · recognition = **juried selection** (INVITED/Tornar — het TheaterFestival; no
  won prizes).
...
The value proposition is delivered in **two registers from one substance**:

- **Institutional register** → P2, P3 ("methodical, durable, fundable" / "track record").
- **Belonging register** → P1, P4 ("is dit voor mij", warmth, "mensen zoals jij").
- **Same proof ladder, different entry tone.** This is the governing "evenwaardig"
  principle expressed in *words and image* — exactly where the client said it matters
  (*"Zeker in tone of voice en in beeld."*).
- **Made concrete per persona:** the **register pair** line in each promise profile above
  states the *same* promise in its institutional vs belonging wording — this section is
  the principle; the profiles are its application.

## Positioning posture (decision D-iv)

The one-line site promise leads **invitation-forward**: lead with the *invitation*; fold
proof in as *social proof*; carry G1's rigor/durability signal **immediately below** the
line (not in it).

- *Illustrative only (EN placeholder — the client voices the final NL):* "An open
  invitation to move together — proven by the people who already did."
- **Why this posture:** it is the boldest honest rebalance away from subsidiedossiertaal
...
line, not in it. **Safeguards:** (a) the durability/rigor signal sits *immediately* below
the line; (b) the jury enters the [proof ladder](30-jobs-to-be-done.md#the-cross-cutting-decision-the-shared-proof-job)
*deep*. Testable **only** at the content-complete-draft review (no second meeting).

## Feeds

→ [User journey](50-user-journey.md) (the value delivered over time, current→future).
